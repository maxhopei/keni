...
### Requirement: Bundled-prompt resolution is enforced — TS string constants only, no path-based loader

The role's `promptResolver` SHALL return a `BundledPrompt = { name: string, body: string }` whose `body` is a TypeScript string constant compiled into the binary. The `resolveBundledPrompt(prompt, expectedName?)` helper SHALL: (a) throw `RoleRuntimeError("empty_prompt_body", ...)` when `prompt.body.length === 0`; (b) when `expectedName` is supplied and `prompt.name !== expectedName`, throw `RoleRuntimeError("prompt_name_mismatch", ...)` whose message names both the expected and the received name; (c) on success, return the validated `BundledPrompt` verbatim. The package's source code under `packages/role-runtimes/src/common/` SHALL NOT contain any path-based prompt-loading function (no `loadPromptFromPath`, no `Deno.readTextFile`, no `import.meta.resolve` resolving a `.keni/`-prefixed path) — every prompt is reachable only as a TS constant. A structural test SHALL assert this by walking source files and rejecting any occurrence of forbidden read primitives (`Deno.readTextFile`, `Deno.readFile`) or any path literal beginning with `.keni/` / `~/.keni/`. (Filesystem *writes* to non-`.keni/` paths — e.g., the default subprocess invoker's `mcpServers` JSON tempfile under `Deno.makeTempFile(...)` — are explicitly permitted; the invariant is "no `.keni/` reads / writes" and "no path-based prompt loader", not "no filesystem IO at all".)
...
- **THEN** no occurrence of `Deno.readTextFile`, `Deno.readFile`, or `import.meta.resolve("./prompts/` followed by a `.txt` / `.md` extension is found in any production file
...
- **WHEN** the source files under `packages/role-runtimes/src/common/` (excluding `*_test.ts` and integration-test fixtures) are scanned for `Deno.readTextFile`, `Deno.readFile`, or any path literal beginning with `.keni/` or `~/.keni/`
- **THEN** no occurrence is found
- **AND** the only filesystem-write primitive that may appear in production source is `Deno.writeTextFile` against a `Deno.makeTempFile`-derived path (the default subprocess invoker's `mcpServers` JSON tempfile, which lives under `${TMPDIR}` — never under `.keni/`)
...
The package SHALL export a `CodingAgentInvoker` interface whose only method is `invoke(invocation: CodingAgentInvocation, lifecycle: CodingAgentLifecycle): Promise<CodingAgentOutcome>`. The cycle SHALL call exactly this method exactly once per cycle (after `session_start` is logged). The package SHALL also export a default factory `createSubprocessCodingAgentInvoker(opts)` whose options bag is `{ cliBinary: string, buildArgs: (invocation, mcpConfigPath) => readonly string[], promptInjection: "stdin" | "arg", mcpConfigPathBuilder?: (invocation) => Promise<string>, graceMs?: number, envAllowlist?: readonly string[], resumeFlag?: string }`. The factory SHALL produce an invoker that: (a) writes the `mcpServers` JSON to a temp file (or invokes `mcpConfigPathBuilder` when supplied) and registers a `try`/`finally` cleanup so the file is removed on cycle exit (success or failure); (b) constructs the `Deno.Command` from `cliBinary` and the result of `buildArgs(invocation, mcpConfigPath)`; (c) writes the prompt to stdin (when `promptInjection === "stdin"`) and closes stdin once written, or relies on the args constructed by `buildArgs` (when `promptInjection === "arg"`); (d) reads stdout / stderr line-by-line and calls `lifecycle.onStdoutLine` / `onStderrLine`; (e) honours `lifecycle.abortSignal` by calling the subprocess utility's `terminate` with the configured `graceMs`; (f) resolves with `{ kind: "completed", exitCode } | { kind: "terminated", exitCode, terminatedBy }`. The invoker SHALL throw on synchronous spawn failures (e.g., binary not found); the cycle catches and surfaces as `spawn_failed`.

#### Scenario: Default factory produces an invoker that runs an actual subprocess and emits stdout lines

- **WHEN** `createSubprocessCodingAgentInvoker({ cliBinary: <path to a Deno-script fixture>, buildArgs: () => [], promptInjection: "stdin" })` is invoked
...
- **WHEN** the default factory is invoked with `cliBinary: "/no/such/binary"`
...
#### Scenario: MCP-config temp file is cleaned up on cycle exit

- **WHEN** the default factory writes an `mcpServers` JSON to a temp file at cycle start
