# spa-ticket-detail Specification

## Purpose

Defines the contract for the **ticket detail** route (`/tickets/:id`) rendered inside `AppShell`'s center `<Outlet />` per `spa-shell`, fed by the same contexts as the board (`spa-board`) and roster (`spa-agent-roster`). Pins fetch-on-mount plus param-change behaviour for the canonical `TicketResponse`, a full-activity client-side filter strategy for status history and comments, title/body PATCH flows with `status_in_patch` avoidance, the collapsible "Advanced: transition" raw-override panel scoped to `SPA_TICKET_STATUS_TRANSITIONS`, posting `ticket_comment` entries through `appendActivity` with size limits, linked PR listing and live `pr.*` refetch rules, debounced `listActivity` refreshes after `activity.appended`, `ticket.updated` refetch handling, and documented loading / error / not-found / disconnected UX plus `TicketDetailView_test.tsx` coverage. Isolated as its own capability so ticket-editing, comment UX, and confirmation-flow replacements (step 25) evolve without entangling PR or activity-log specs.
