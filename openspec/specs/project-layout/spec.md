...
Defines the on-disk bootstrap contract for a Keni project: the exact shape of `<root>/.keni/` and `<home>/.keni/` after `keni init` runs, the UX and exit conditions of the `keni init` command itself, the rules for `project_id` generation, the `.gitignore` merge policy, the initial-commit policy, the idempotency contract for re-runs, the partial-state repair behaviour, and the explicit list of files / directories the prototype owns under `.keni/` (vs. those reserved for MVP steps 14 and 15). This capability does not describe how individual files are read or written (that responsibility lies with the `storage` capability); it describes the shape of the project after init and the deterministic command flow that produces it. Step 13 (`keni start`) and every later step that operates against `.keni/` reads this spec to know what they can rely on existing.
