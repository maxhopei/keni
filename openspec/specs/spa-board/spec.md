# spa-board Specification

## Purpose

Defines the contract for the **kanban board** — the real index route rendered in `AppShell`'s center `<Outlet />` beside the agent roster pinned by `spa-agent-roster` and on top of the same `apiClient` / `eventsClient` seam mandated by `spa-shell`. Pins the twelve-column `TicketStatus` layout and horizontal scroll behaviour, card render shape and navigation into `spa-ticket-detail`, drag-and-drop transitions with server-confirmed state and documented failure UX, live updates driven by `ticket.created` / `ticket.updated`, reconnect-tier refetch behaviour, inline ticket creation plus post-create navigation, the SPA mirror of the server's status graph (`statusGraph.ts` + drift tests), component tests mounting in-memory fakes, and explicit loading / empty / error / disconnected states consistent with sibling panels. Split out from the shell roster so board-only futures (filters, virtualisation, alternative DnD) touch exactly this capability rather than reopening routing or roster contracts.
