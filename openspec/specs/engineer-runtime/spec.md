...
For the `"claude"` entry, `cliBinary` SHALL be `"claude"`, `promptInjection` SHALL be `"stdin"`, `resumeFlag` SHALL be `"--resume"`, `mcpConfigStrategy.kind` SHALL be `"tempfile-json"`, and `buildArgs` SHALL produce an argv that includes `["--mcp-config", mcpConfigPath]`, the documented non-interactive flag `"--print"`, and the consecutive pair `["--permission-mode", "bypassPermissions"]`. The `--permission-mode bypassPermissions` switch is the structural analogue of `cursor-agent`'s `--approve-mcps --trust` and is required for headless operation: without it, every MCP tool call (and every `Bash`, `Read`, `Edit`, etc.) blocks on an approval prompt that no human can answer in the engineer loop, the subprocess exits emitting "I need permission to use the MCP tools" on stdout, and the cycle does no useful work. The mode is documented for "isolated containers and VMs" by the [Claude Code permission-modes docs](https://docs.anthropic.com/en/docs/claude-code/permission-modes), which matches the engineer's per-agent workspace (a sparse-checkout clone under `~/.keni/workspaces/<projectId>/<agentId>/` with its own git identity that excludes `.keni/`). The argv SHALL NOT contain `--interactive` or any flag that would block on a TTY. Coverage SHALL be `"tested"` (verified against the documented `claude --help` and the unit test in `codingAgentCliRegistry_test.ts`).
...
#### Scenario: The `claude` entry's `buildArgs` uses `--mcp-config`, `--print`, and `--permission-mode bypassPermissions`, and its strategy is tempfile-json
...
- **AND** the argv contains the consecutive pair `["--permission-mode", "bypassPermissions"]`
