# runtime-po-stub Specification

## Purpose

`@keni/runtime-po` is a deliberately-stubbed role package whose only job is to prove the polymorphic role plug-in model — bind a second role's `wire(input)` under `Role: "po"`, register an `AgentRunner` whose precheck always skips, and validate that the orchestration server's `roleWires` dispatch ticks both engineer and PO without any role-specific code in the server. The PO role's real behaviour (backlog grooming, ticket triage, spec authoring, …) ships in a future change; until then, every PO tick precheck-skips with `reason: "po_not_implemented"`.

Created by archiving change `split-role-runtimes-package`.
