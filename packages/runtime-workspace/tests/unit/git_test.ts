...
import { GitWorkspaceProvisioner } from "../../src/git.ts";
import type { WorkspaceLogger } from "../../src/interface.ts";
import { WorkspaceProvisioningError } from "../../src/interface.ts";

// The engineer's documented sparse-checkout pattern, inlined here for
// the production-default tests. The constant lives in
// `@keni/runtime-engineer` as `ENGINEER_SPARSE_CHECKOUT_PATTERN`; this
// test file shadows it locally to avoid a runtime-workspace ->
// runtime-engineer dependency edge.
const ENGINEER_SPARSE_CHECKOUT_PATTERN: readonly string[] = ["/*", "!.keni/"];
const ENGINEER_SPARSE_FILE_BODY = "/*\n!.keni/\n";
...
Deno.test("ensureProvisioned: first-time provisioning produces a sparse clone with .keni absent", async () => {
  ...
  try {
    ...
    assertEquals(pattern, ENGINEER_SPARSE_FILE_BODY);
  } finally {
...
Deno.test("ensureProvisioned: drifted sparse pattern is repaired", async () => {
  ...
  try {
    ...
    assertEquals(await Deno.readTextFile(patternFile), ENGINEER_SPARSE_FILE_BODY);
    assertEquals(await exists(joinPath(workspace, ".keni")), false);
  } finally {
    await teardown(home, repo);
  }
