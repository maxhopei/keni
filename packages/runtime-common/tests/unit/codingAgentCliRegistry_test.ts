...
} from "../../src/codingAgentCliRegistry.ts";
import {
  createSubprocessCodingAgentInvoker,
  type SubprocessCodingAgentInvokerOpts,
} from "../../src/codingAgentInvoker.ts";
import type { CodingAgentInvocation, McpServerConfig } from "../../src/types.ts";
...
Deno.test("codingAgentCliRegistry is referentially stable across imports", async () => {
  ...
  const reimport = await import("../../src/codingAgentCliRegistry.ts");
...
const CODING_AGENT_CLIS_DIR = fromFileUrl(
  new URL("../../src/codingAgentClis", import.meta.url),
);
...
Deno.test(
  ...
  async () => {
    const registryPath = fromFileUrl(
      new URL("../../src/codingAgentCliRegistry.ts", import.meta.url),
    );
...
Deno.test(
  ...
  async () => {
    ...
    for (const [file, exportName] of expectedExports.entries()) {
      const mod = await import(`../../src/codingAgentClis/${file}`);
