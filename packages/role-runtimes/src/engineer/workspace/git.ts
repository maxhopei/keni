...
/**
 * The exact two-line sparse-checkout pattern the engineer runtime
 * requires. Pinned by `git_test.ts` and re-asserted on every
 * re-provisioning to repair drift. Each line is `\n`-terminated; no
 * trailing blank line.
 */
export const SPARSE_CHECKOUT_PATTERN = "/*\n!.keni/\n";
...
export class GitWorkspaceProvisioner implements WorkspaceProvisioner {
  ...
  async ensureProvisioned(
    ...
    if (!exists) {
      await this.firstTimeProvision(workspacePath, projectRepoPath, agentId);
      this.logger.log("info", "engineer.workspace_provisioned", {
        agent: agentId,
        path: workspacePath,
      });
    } else {
      await this.repairProvisioned(workspacePath, sparsePatternFile, agentId);
      this.logger.log("debug", "engineer.workspace_verified", {
        agent: agentId,
        path: workspacePath,
      });
    }

    await assertKeniAbsent(workspacePath);
  }
  ...
  private async firstTimeProvision(
    ...
    await writeSparsePattern(workspacePath);
  ...
  private async repairProvisioned(
    ...
    if (currentPattern !== SPARSE_CHECKOUT_PATTERN) drift = true;
    if (drift) {
      await writeSparsePattern(workspacePath);
      await this.runGit(
        ["-C", workspacePath, "sparse-checkout", "reapply"],
...
async function assertKeniAbsent(workspacePath: string): Promise<void> {
  const keniPath = joinPath(workspacePath, ".keni");
  try {
    await Deno.lstat(keniPath);
  } catch (err) {
    if (err instanceof Deno.errors.NotFound) return;
    throw err;
  }
  throw new WorkspaceProvisioningError(
    ...
    `.keni/ unexpectedly present at ${keniPath} after sparse checkout`,
    { path: keniPath },
  );
}

async function writeSparsePattern(workspacePath: string): Promise<void> {
  ...
  await Deno.writeTextFile(file, SPARSE_CHECKOUT_PATTERN);
}
