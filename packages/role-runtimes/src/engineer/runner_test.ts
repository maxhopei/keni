...
import { FakeWorkspaceProvisioner } from "./workspace/fakes/fakeWorkspaceProvisioner.ts";
import { type WorkspaceLogger, WorkspaceProvisioningError } from "./workspace/interface.ts";
...
Deno.test("precheck calls pullMain first, before any HTTP query", async () => {
  ...
  assertEquals(provisioner.calls[0]?.method, "pullMain");
  assertEquals(provisioner.calls[0]?.args, ["p1", "alice"]);
