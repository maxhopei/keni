 ...
 *   `packages/role-runtimes/tests/fixtures/fake-coding-agent.ts`,
...
import { FakeWorkspaceProvisioner } from "../../fakes/engineer/workspace/fakeWorkspaceProvisioner.ts";
...
const COMMON_DIR = fromFileUrl(new URL("../../../src/common/", import.meta.url));
...
Deno.test("structural — packages/role-runtimes/src/common/ has no role-keyed conditionals", async () => {
  const files = await listProductionSourceFiles();
...
Deno.test("structural — packages/role-runtimes/src/common/ does not import the MCP SDK", async () => {
  const files = await listProductionSourceFiles();
  for (const file of files) {
    const stripped = stripComments(await Deno.readTextFile(file));
